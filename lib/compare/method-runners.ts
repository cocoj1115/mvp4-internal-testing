...
async function runMethod1(input: MethodRunInput): Promise<MethodRunResult> {
  ...
  const systemPrompt = [
    ...
    "  Write exactly 1 declarative sentence. Nothing more.",
    ...
    "  After the opener: echo the specific concept using the student's own wording where possible, then name what it correctly identifies or explains.",
    "  Template: '[Opener] [student's key phrase] correctly identifies/explains [what the concept does or is].'",
    "  Example: 'Exactly right — amino acid order determines the folding pattern and 3D shape of the protein.'",
    "  No questions, no 'but', no 'however', no additional information, no hints at the next part.",
    "",
