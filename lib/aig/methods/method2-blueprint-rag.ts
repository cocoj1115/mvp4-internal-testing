...
export function buildBlueprintPrompt(
  ...
  const priorSeqSection = priors
    .map((p) => `${p.item}: ${p.sequence.map((t, i) => `${["A", "B", "C"][i]}=${t}`).join(" -> ")}`)
    .join("\n");
...
export function buildItemPrompt(
  ...
  const system = [
    ...
    "   - 'diagram': structure, cycle, or process that must be described textually -> provide diagram_spec; this is shown as a text-only diagram spec, not image generated",
    "   - 'scenario': a text-only stimulus with named organisms, conditions, and concrete observations -> provide scenario_text",
    "   - 'illustration': requires a realistic image (organism, habitat, cell) -> provide illustration_prompt; the app will automatically send this prompt to the image generation model",
    ...
    `7. Stimulus constraint: ${forcedStimulusInstruction(options)}`,
    options?.revisionInstructions
      ? `8. Revision instructions from style check: ${options.revisionInstructions}`
      : "",
    "",
